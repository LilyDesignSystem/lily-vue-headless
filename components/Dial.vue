<script setup lang="ts">
    ...
    //
    // Syntax:
    //   <Dial label="Volume" v-model min={0} max={100} />
    //
    // Examples:
    //   <!-- Brightness dial with step of 5 -->
    //   <Dial label="Brightness" v-model={brightness} min={0} max={100} step={5} />
    //
    //   <!-- Disabled dial -->
    ...
    function onkeydown(event: KeyboardEvent) {
        ...
        switch (event.key) {
            case "ArrowRight":
            case "ArrowUp":
                modelValue.value = clamp(modelValue.value + props.step * multiplier);
